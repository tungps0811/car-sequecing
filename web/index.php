...
$config = [
  'default_controller' => 'Projet',
  'default_method' => 'index',
  'core_classes'=> ['Controller','Model','Loader'],
  'models'=>['Projet']
];
...
function create_controller($controller_name) {
  ...
  if (!file_exists($controller_filename)) { throw new Exception("URL inconnue."); }
  include $controller_filename;
  if (!class_exists($controller_classname)) { throw new Exception("URL inconnue."); }
  return new $controller_classname();
}

function call_controller_method($controller, $method_name, $parameters) {
 $reflectionObject = new ReflectionObject($controller);
  if (! $reflectionObject->hasMethod($method_name)) { throw new Exception("URL inconnue."); }
  $reflectionMethod = $reflectionObject->getMethod($method_name);
  if ($reflectionMethod->getNumberOfParameters()!=count($parameters)){ throw new Exception("URL inconnue."); }
